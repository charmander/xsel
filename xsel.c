...
usage (void)
{
  puts ("Usage: xsel [options]");
  puts ("Manipulate the X selection.\n");
  puts ("By default the current selection is output and not modified if both");
  puts ("standard input and standard output are terminals (ttys).  Otherwise,");
  puts ("the current selection is output if standard output is not a terminal");
  puts ("(tty), and the selection is set from standard input if standard input");
  puts ("is not a terminal (tty). If any input or output options are given then");
  puts ("the program behaves only in the requested mode.\n");
  puts ("If both input and output is required then the previous selection is");
  puts ("output before being replaced by the contents of standard input.\n");
  puts ("Input options");
  puts ("  -a, --append          Append standard input to the selection");
  puts ("  -f, --follow          Append to selection as standard input grows");
  puts ("  -z, --zeroflush       Overwrites selection when zero ('\\0') is received");
  puts ("  -i, --input           Read standard input into the selection\n");
  puts ("Output options");
  puts ("  -o, --output          Write the selection to standard output\n");
  puts ("Action options");
  puts ("  -c, --clear           Clear the selection");
  puts ("  -d, --delete          Request that the selection be cleared and that");
  puts ("                        the application owning it delete its contents\n");
  puts ("Selection options");
  puts ("  -p, --primary         Operate on the PRIMARY selection (default)");
  puts ("  -s, --secondary       Operate on the SECONDARY selection");
  puts ("  -b, --clipboard       Operate on the CLIPBOARD selection\n");
  puts ("  -k, --keep            Do not modify the selections, but make the PRIMARY");
  puts ("                        and SECONDARY selections persist even after the");
  puts ("                        programs they were selected in exit.");
  puts ("  -x, --exchange        Exchange the PRIMARY and SECONDARY selections\n");
  puts ("X options");
  puts ("  --display displayname");
  puts ("                        Specify the connection to the X server");
  puts ("  -t ms, --selectionTimeout ms");
  puts ("                        Specify the timeout in milliseconds within which the");
  puts ("                        selection must be retrieved. A value of 0 (zero)");
  puts ("                        specifies no timeout (default)\n");
  puts ("Miscellaneous options");
  puts ("  -l, --logfile         Specify file to log errors to when detached.");
  puts ("  -n, --nodetach        Do not detach from the controlling terminal. Without");
  puts ("                        this option, xsel will fork to become a background");
  puts ("                        process in input, exchange and keep modes.\n");
  puts ("  -h, --help            Display this help and exit");
  puts ("  -v, --verbose         Print informative messages");
  puts ("  --version             Output version information and exit\n");
  puts ("Please report bugs to <conrad@vergenet.net>.");
}
